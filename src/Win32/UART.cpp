...
quint32 UARTImpl::write(const uint8_t *buffer, quint32 size, STATUS *status)
{
    ...
    if(overlapped.hEvent == NULL)    // failed creating the event
    {
        stat = STATUS_DEVICE_WRITE_FAILED;
    }
    else if(WriteFile(deviceHandle_, buffer, size, &bytesSend, &overlapped))
    {
        stat = STATUS_SUCCESS;
    }
    else
...
quint32 UARTImpl::read(uint8_t *buffer, quint32 size, STATUS *status)
{
    ...
    else if(ReadFile(deviceHandle_, buffer, size, &bytesRead, &overlapped))
    {
        stat = STATUS_SUCCESS;
    }
    else
